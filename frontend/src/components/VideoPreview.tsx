  ...
  return (
    <div className="mt-12 flex flex-col md:flex-row gap-8 animate-in slide-in-from-top-4 fade-in duration-500">
      ...
      <div className="flex-1 flex flex-col">
        <h3 className="text-xl font-semibold text-white leading-tight">{videoInfo.title}</h3>
        <p className="flex items-center gap-2 text-zinc-400 text-sm mt-3 font-medium">
          <span className="bg-zinc-800/50 px-2 py-0.5 rounded-lg text-zinc-500 text-xs">{videoInfo.uploader}</span>
        ...
        <div className="mt-2 flex flex-col sm:flex-row gap-4 items-end">
          ...
          >
            ...
            <span>{videoInfo.is_playlist ? `Save Playlist (${videoInfo.playlist_count} videos)` : 'Save Video'}</span>
          </button>
        </div>
      </div>
    </div>
